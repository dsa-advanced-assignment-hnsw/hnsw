...
Hệ thống tìm kiếm đa phương thức được thiết kế theo kiến trúc client-server với ba dịch vụ backend độc lập và một frontend thống nhất. \Cref{fig:system_architecture} mô tả kiến trúc tổng thể của hệ thống.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/system_architecture.png}
    \caption{Kiến trúc hệ thống tìm kiếm đa phương thức sử dụng HNSW}
...
\begin{itemize}
    \item Image Search API: \texttt{http://localhost:5000}
    \item Paper Search API: \texttt{http://localhost:5001}
    \item Medical Search API: \texttt{http://localhost:5002}
\end{itemize}

\subsection{Thành phần Backend}
Backend được triển khai bằng Flask (Python) với ba dịch vụ độc lập:

\subsubsection{Image Search Service (Port 5000)}
\begin{itemize}
    \item \textbf{Mô hình}: OpenAI CLIP (ViT-B/32)
    \item \textbf{Embedding dimension}: 512
    \item \textbf{Dataset}: Open Images V7, 100,000+ hình ảnh
    \item \textbf{Storage}: HDF5 file (\texttt{Images\_Embedbed\_0-100000.h5})
    \item \textbf{Features}: Hỗ trợ hình ảnh từ nhiều nguồn (Flickr, Pinterest, Google Images)
\end{itemize}

\subsubsection{Paper Search Service (Port 5001)}
\begin{itemize}
    \item \textbf{Mô hình}: Sentence Transformers (all-roberta-large-v1)
    \item \textbf{Embedding dimension}: 1024
    \item \textbf{Dataset}: arXiv papers, 100,000+ tài liệu
...
\subsubsection{Medical Search Service (Port 5002)}
\begin{itemize}
    \item \textbf{Mô hình}: BiomedCLIP (microsoft/BiomedCLIP-PubMedBERT\_256-vit\_base\_patch16\_224)
    \item \textbf{Embedding dimension}: 512
    \item \textbf{Dataset}: FracAtlas bone fractures, 3,400+ hình ảnh X-quang
    \item \textbf{Storage}: HDF5 file (\texttt{Medical\_Fractures\_Embedded.h5})
...
\begin{itemize}
    \item \textbf{M (số kết nối tối đa)}: 200 - Đảm bảo độ chính xác cao
...
\begin{itemize}
    ...
    \item Kích thước file nhỏ: ~5MB cho 100K hình ảnh, ~4GB cho 1M tài liệu
\end{itemize}
